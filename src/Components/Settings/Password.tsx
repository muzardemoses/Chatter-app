...
export const Password = () => {
    ...
    const [isConnectedWithPassword, setIsConnectedWithPassword] = useState(false)

    const [createdPassword, setCreatedPassword] = useState('')
    const [confirmCreatedPassword, setConfirmCreatedPassword] = useState('')

    const [showCreatedPassword, setShowCreatedPassword] = useState(false)
    const [showConfirmCreatedPassword, setShowConfirmCreatedPassword] = useState(false)

    useEffect(() => {
        if (currentUser) {
            if (currentUser.providerData.some((data: any) => data.providerId === 'password')) {
                setIsConnectedWithPassword(true)
            } else {
                setIsConnectedWithPassword(false)
            }
        }
    }, [currentUser])
                        ...
                        <div className='flex flex-col gap-8'>
                            ...
                            <div className='flex flex-col gap-5 w-[90%]'>
                                <div className="relative flex flex-col gap-3 md:gap-2">
                                    ...
                                    />
                                    <img
                                        src={showCreatedPassword ? hideImg : showImg}
                                        alt="show password"
                                        className="absolute right-4 bottom-1.5 transform -translate-y-1/2 cursor-pointer md:bottom-0"
                                ...
                                <div className="relative flex flex-col gap-3 md:gap-2">
                                    ...
                                    <input
                                        type={showConfirmCreatedPassword ? "text" : "password"}
                                        id="confirm-password"
                                        placeholder="Confirm your new password"
                                        className='w-full text-base font-normal border border-gray-300 shadow rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent transition ease-in-out duration-500 md:p-2'
                                    ...
                                    />
                                    <img
                                        src={showConfirmCreatedPassword ? hideImg : showImg}
                                        alt="show password"
                                        className="absolute right-4 bottom-1.5 transform -translate-y-1/2 cursor-pointer md:bottom-0"
                                        onClick={() => setShowConfirmCreatedPassword(!showConfirmCreatedPassword)}
                                    />
                                </div>
                            </div>
                            {showEmailInfo && (
                                <div className='flex flex-col gap-0.5 mt-1'>
                                    <div className='flex items-center gap-2'>
                                        <p className='h-2 w-2 bg-blue-800 rounded-full'></p>
                                        <p className="text-sm text-blue-600">
                                            You were asked to confirm your password or re-authenticate your account because you are required to do so before adding a password to your account
                                        </p>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <p className='h-2 w-2 bg-green-800 rounded-full'></p>
                                        <p className="text-sm text-green-700">
                                            You can proceed to add a password to your account
                                        </p>
                                    </div>
                                </div>
                            )}
